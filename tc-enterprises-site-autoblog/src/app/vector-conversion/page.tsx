      ...
      <section className="py-12">
        <div className="text-center mb-10">
          ...
          <p className="text-lg text-muted dark:text-muted-foreground max-w-3xl mx-auto">
            Raster originals on the left. Engraving-ready vector output on the right.
          </p>
        </div>

        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {[
            { label: 'Ladybug Logo', before: '/examples/ladybug-before.png', after: '/examples/ladybug-after.png', type: 'EPS — Laser' },
            { label: 'HL Monogram', before: '/examples/hl-before.jpg', after: '/examples/hl-after.png', type: 'PLT — Rotary' },
            { label: 'House Logo', before: '/examples/house-before.png', after: '/examples/house-after.png', type: 'PLT — Rotary' },
          ].map(({ label, before, after, type }) => (
            <div key={label} className="card overflow-hidden group">
              <div className="flex items-stretch divide-x divide-gray-200 dark:divide-white/10">
                <div className="flex-1 flex flex-col items-center p-3 gap-2">
                  <span className="text-xs font-semibold uppercase tracking-wide text-red-500 dark:text-red-400">Before</span>
                  <div className="overflow-hidden rounded">
                    <img
                      src={before}
                      alt={`${label} — original raster`}
                ...
                <div className="flex-1 flex flex-col items-center p-3 gap-2">
                  <span className="text-xs font-semibold uppercase tracking-wide text-green-600 dark:text-green-400">After</span>
                  <div className="overflow-hidden rounded">
                    <img
                      src={after}
                      alt={`${label} — engraving-ready vector`}
                      className="w-full h-32 object-contain transition-transform duration-300 group-hover:scale-105"
                    />
                  </div>
                </div>
              </div>
              <div className="px-4 pb-3 pt-1 border-t border-gray-100 dark:border-white/10 flex justify-between items-center">
                <span className="text-xs font-medium text-foreground dark:text-foreground-dark">{label}</span>
                <span className="text-xs text-muted dark:text-muted-foreground">{type}</span>
              </div>
            </div>
          ))}
        </div>
